...
def guardar_partida_db(nombre_entrenador, pokemon_p, lista_atrapados, p_id=None):
    try:
        ...
        if p_id:
            ...
            cursor.execute("INSERT INTO partidas (entrenador, fecha_guardado) VALUES (?, ?)",
                           (nombre_entrenador, fecha_actual))
            partida_id = cursor.lastrowid

        tipo_str = "Agua"
        if isinstance(pokemon_p, PokemonFuego):
            tipo_str = "Fuego"
        elif isinstance(pokemon_p, PokemonElectrico):
            tipo_str = "Electrico"
        elif isinstance(pokemon_p, PokemonHierba):
            tipo_str = "Hierba"
        ...
        cursor.execute('''INSERT OR REPLACE INTO pokemon_actual (partida_id, nombre, tipo, nivel, vida, defensa, ataque, evolucion, descripcion) 
                          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                       (partida_id, pokemon_p.nombre, tipo_str, pokemon_p.nivel, pokemon_p.vida, pokemon_p.defensa,
                        pokemon_p.ataque, pokemon_p.evolucion, pokemon_p.descripcion))

        cursor.execute("DELETE FROM pokemon_atrapados WHERE partida_id = ?", (partida_id,))
        for p in lista_atrapados:
            t_cap = "Agua"
            if isinstance(p, PokemonFuego):
                t_cap = "Fuego"
            elif isinstance(p, PokemonElectrico):
                t_cap = "Electrico"
            elif isinstance(p, PokemonHierba):
                t_cap = "Hierba"

            cursor.execute(
                '''INSERT INTO pokemon_atrapados (partida_id, nombre, tipo, nivel, vida, defensa, ataque, evolucion, descripcion)
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
